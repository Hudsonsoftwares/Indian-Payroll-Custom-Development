...
# Check if table_id or restaurant table exists
cur.execute("SELECT id, name FROM restaurant_table LIMIT 10")
tables = cur.fetchall()
table_map = {t[1]: t[0] for t in tables} if tables else {}
print("Restaurant tables found:", tables)

for idx, o in enumerate(sample_orders):
    ref = f"Order 0000{idx+1}-001-000{idx+1}"
    tbl_id = table_map.get(o['table'])
    
    if tbl_id:
        cur.execute("""
            INSERT INTO pos_order (name, session_id, company_id, pos_reference, table_id, customer_count, amount_total, amount_tax, amount_paid, amount_return, state, create_uid, write_uid, create_date, write_date)
            VALUES (%s, %s, %s, %s, %s, %s, 50.0, 5.0, 50.0, 0.0, 'paid', 1, 1, %s, %s)
            RETURNING id
        """, (ref, session_id, company_id, ref, tbl_id, o['guests'], o['created'], o['created']))
    else:
        cur.execute("""
    ...
    cur.execute("""
        INSERT INTO pos_prep_order (pos_order_id, display_id, stage_id, fulfillment_type, create_uid, write_uid, create_date, write_date)
        VALUES (%s, 1, %s, 'dine_in', 1, 1, %s, %s)
        RETURNING id
    """, (pos_order_id, o['stage_id'], o['created'], o['created']))
